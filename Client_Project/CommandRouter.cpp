...
void handlePasv(const std::vector<std::string>& args, ServerSession& session) {
    ...
    sockaddr_in local_addr; socklen_t addr_len = sizeof(local_addr);
    getsockname(session.tcp_sock, (sockaddr*)&local_addr, &addr_len);
    ...
    std::string reply = "Entering Passive Mode (" + ip_str + "," +
        std::to_string(session.data_port / 256) + "," +
        std::to_string(session.data_port % 256) + ").";
...
void handleRetr(const std::vector<std::string>& args, ServerSession& session) {
    ...
    std::thread([&session, full_path]() {
        ...
        // CHỈ gửi thông báo khi thành công hoặc lỗi ngoài ý muốn.
        // NẾU LÀ ABOR: Bỏ qua không gửi 426 nữa vì handleAbort đã gửi 225 rồi.
        if (ok) {
            send_tcp_reply(session.tcp_sock, 226, "Transfer complete.", &session.tcp_mutex);
        }
        else if (!session.abortRequested.load()) {
            send_tcp_reply(session.tcp_sock, 451, "Error transmitting file.", &session.tcp_mutex);
        }
...
void handleStorAppeStou(const std::vector<std::string>& args, ServerSession& session, const std::string& cmd) {
    ...
    std::thread([&session, full_path, is_append, cmd, filename]() {
        ...
        // CHỈ gửi thông báo khi thành công hoặc lỗi ngoài ý muốn.
        // NẾU LÀ ABOR: Bỏ qua không gửi 426 nữa.
        if (ok) {
            ...
            send_tcp_reply(session.tcp_sock, 451, "Error processing file.", &session.tcp_mutex);
        }
